...
### 摘要
完成 Dashboard 下一阶段的设计方案，目标是在现有 Terminal Instrument 视觉体系上增加市场数据图表、组合基准比较、风险趋势、统一事件时间线和克制动效，提高信息密度与 Agent 建议的证据完整性。本次仅新增设计文档，未修改功能代码。

### 已完成
- 新增根目录文档 `DASHBOARD_MARKET_COCKPIT_DESIGN.md`。
- 确定“市场总览 + 组合驾驶舱”双模式信息架构。
- 规划市场状态栏、市场走势比较、市场环境矩阵、组合归因、Greeks 趋势、事件时间线和 Agent 建议中心。
- 明确行情缓存、SQLite 快照、数据新鲜度、错误降级和 Agent 数据版本审计原则。
- 补充完整工具运用方案：设计与开发 Skills、React/Next.js、Recharts、Motion、IBKR/Yahoo 数据、SQLite、Agent 工具链、浏览器和自动化验收工具。
- 给出六阶段实施顺序及可衡量的验收标准。

### 后续修改建议
- 实施前先把方案拆成 TDD 任务计划，再开始修改功能代码。
- 第一阶段优先完成数据契约与 Market Overview API，避免前端先依赖临时假数据。
- Motion 只用于状态变化和信息层级，GSAP 暂不进入主 Dashboard 技术栈。
